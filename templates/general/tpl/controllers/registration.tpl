				...
				<tr>
					...
					<td class="field">
						{if $account_types|@count > 1}
							...
							{/foreach}
						{else}
							<span class="default_size">{$account_types.0.name}</span>
							<select name="profile[type]" class="hide">
									<option value="{$account_types.0.ID}" selected="selected">{$account_types.0.name}</option>
							</select>
							<div class="qtip_cont">{$account_types.0.desc}</div><img title="" class="qtip" src="{$rlTplBase}img/blank.gif" alt="" />
						{/if}
					</td>
				</tr>
				</table>
