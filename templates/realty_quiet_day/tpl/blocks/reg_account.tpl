	...
	<tr class="tmp">
		...
		<td class="field" id="sf_field_{$field.Key}">
			...
			{elseif $field.Type == 'checkbox'}
				...
				{foreach from=$field.Values item='option' key='key' name='checkboxF'}
					<td {if $smarty.foreach.checkboxF.total > 5}style="width: 33%"{/if}>
						<label><input type="checkbox" value="{$key}" {if is_array($fVal.$fKey)}{foreach from=$fVal.$fKey item='chVals'}{if $chVals == $key}checked="checkbox"{/if}{/foreach}{else}{foreach from=$field.Default item='chDef'}{if $chDef == $key}checked="checkbox"{/if}{/foreach}{/if} name="account[{$field.Key}][{$key}]" /> {$lang[$option.pName]}</label><br>
					</td>
			...
			{elseif $field.Type == 'accept'}
				<textarea style="width: 80%;" rows="6" cols="" readonly="readonly" class="text" name="{$field.Key}">{$field.default}</textarea><br />
