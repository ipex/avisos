	...
	<tr>
		...
		{if $blocks.left}	
			<td class="left{if !$blocks.right} wide{/if}">
				{assign var='side_first' value=true}
				{foreach from=$blocks item='block'}
				{if $block.Side == 'left'}
					{include file='blocks'|cat:$smarty.const.RL_DS|cat:'blocks_manager.tpl' block=$block first=$side_first}
					{assign var='side_first' value=false}
				{/if}
				{/foreach}
			</td>
		{/if}
