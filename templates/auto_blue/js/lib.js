...
*
**/
var featuredGallery = function(){
	var width = $('div.featured_gallery div.preview').width(),
		item_width = $('div.featured_gallery ul.featured > li:first img').width(),
		min_padding = 5,
		new_padding = 10,
	...
	/* correct preview container height */
	$('div.featured_gallery div.preview').height(width/2);
	$('div.featured_gallery div.preview > a').html('<div></div>');
	$('div.featured_gallery div.preview > a > div').height(width/2);
	
	/* correct items position */
	var visible_items = Math.floor(width / (item_width + (min_padding / (count - 1))))
	if ( visible_items < count )
	{
		new_padding = Math.floor((width - (visible_items * item_width)) / (visible_items - 1));
		var p = rlLangDir == 'rtl' ? 'paddingLeft' : 'paddingRight';
		$('div.featured_gallery ul.featured > li > div.content').css(p, new_padding);
	}
	
	/* add red mask */
	$('div.featured_gallery ul.featured li a img').each(function(){
		var width = $(this).width();
		var height = $(this).height();
		$(this).closest('div.content').find('> ul > li').width($(this).closest('div.content').find('> ul > li').width() - 21);
		$(this).closest('div.content').find('> ul > li > *').each(function(){
			$(this).replaceWith(this.childNodes);
		});
		$(this).closest('div.content').append('<div class="mask" style="width: '+width+'px;height: '+height+'px;"><span class="triangle"></span><span class="more"><a href="#">'+lang['menu_more']+' &raquo;</a></span></div>');
		$(this).closest('div.content').find('> ul > li:gt(1)').remove();
		$(this).closest('div.content').find('div.mask').prepend($(this).closest('div.content').find('> ul'));
		var href = $(this).closest('a').attr('href');
		$(this).closest('div.content').find('span.more a').attr('href', href);
		$(this).closest('a').replaceWith(this);
	});
	
	var onLoadHandler = function(obj, response, loaded){
		$('div.featured_gallery div.preview > a > div').css('backgroundImage', 'url('+response+')');
		if ( !loaded )
		{
			$('div.featured_gallery div.preview > a > div').fadeIn();
		}
		$('div.featured_gallery div.preview > a').attr('href', $(obj).find('> div').find('span.more a').attr('href'));
		$(obj).attr('accesskey', response);
	}
	
	var onClickHandler = function(nav){
		var obj = 'div.featured_gallery ul.featured > li:eq('+index+')';
		var loaded = $(obj).attr('accesskey') ? true : false;
		
		/* restopre context */
		$('div.featured_gallery ul.featured li div.mask').hide();
		if ( !loaded )
		{
			$('div.featured_gallery div.preview > a > div').hide();
		}
		
		/* handler */
		var src = $(obj).find('div.content img').attr('src');
		src = src.replace(rlConfig['files_url'], '');
		
		$(obj).find('> div').find('div.mask').fadeIn('fast');
		
		if ( loaded )
		{
			onLoadHandler(obj, $(obj).attr('accesskey'), loaded);
		}
		else if ( src )
		{
			$.getJSON(rlConfig['tpl_base'] +'request.ajax.php', {mode: 'photo', item: src, lang: rlLang}, function(response){
				if( response != '' && response != null )
				{
					var img = new Image();
					img.onload = function(){
//						setTimeout(function(){
//							onLoadHandler(obj, response, loaded);
//						}, 500)
						onLoadHandler(obj, response, loaded);
					}
					img.src = response;
		...
		if ( visible_items < count )
		{
			var tmp_tile = (index + 1) - visible_items < 0 ? 0 : (index + 1) - visible_items;
			tile = tile ? tile : tmp_tile;
			
			var current = tile > 0 ? (index + 1) - tile : (index + 1);
			
			if ( (current == visible_items && index + 1 != count && !nav) || (current == visible_items && nav) )
			{
				if ( !nav ) tile++;
				var set_pos = (item_width + new_padding) * tile;
				$('div.featured_gallery ul.featured > li:first').animate({marginLeft: '-'+ set_pos +'px'}, 200);
			}
			else if ( (current == 1 && index > 0 && !nav) || (current == 1 && nav) )
			{
				if ( !nav ) tile--;
				var set_pos = (item_width + new_padding) * tile;
				$('div.featured_gallery ul.featured > li:first').animate({marginLeft: '-'+ set_pos +'px'}, 200);
	...
	$('div.featured_gallery div.preview > div.next, div.featured_gallery div.preview > div.prev, div.featured_gallery ul.featured li div.content img').click(function(){
		clicked = true;
		clearTimeout(timer);
	});
	
	/* featured thumbnail click handler */
	$('div.featured_gallery ul.featured li div.content img').click(function(){
		index = $('div.featured_gallery ul.featured > li').index($(this).closest('li'));
		onClickHandler();
	});
	
	/* nav arrow click handler */
	$('div.featured_gallery div.preview > div.next').click(function(){
		var nav = false;
		if ( (count - 1) == index )
		{
			index = 0;
	...
	$('div.featured_gallery div.preview > div.prev').click(function(){
		var nav = false;
		if ( 0 == index )
		{
			index = count - 1;
			tile = count - visible_items;
			nav = true;
		}
		else
		{
...
var tfPath = new Array();
	...
	tfPath['price_tag_to'] = 'div.top-layer div.fields tr.price > td:last',
...
var transformToList = function(index){
	...
	/* move price tag */
	$(item).find('div.top-layer > table > tbody > tr:last').after('<tr><td class="caption" valign="bottom"><table class="nav"><tr><td valign="bottom"><span class="price"></span></td><td class="ralign" valign="bottom"></td></tr></table></td></tr>');
	$(item).find(tfPath['price_tag_to']).hide();
	$(item).find(tfPath['price_tag_from']).find('span.price').prepend($(item).find(tfPath['price_tag_to']).html());
	...
	$(item).find(tfPath['category_name_to']).append($(item).find('div.category-name').removeClass('text-overflow'));
	...
	{
		var comma = i == 0 || (i - 1 == price_index && i == 1)  ? '' : ', ';
		var title = rlConfig['sf_display_fields'] && names[i] ? ' title='+ names[i] : '';
		var hide = i == price_index ? 'class="hide"' : '';
		var new_value = '<span'+title+' '+hide+'>'+ comma + fields[i] +'</span> ';
...
var transformToTable = function(index){
	...
	/* remove price tag */
	$(item).find(tfPath['price_tag_from']).find('span.price').remove();
	...
	category = $(item).find(tfPath['category_name_from']);
	...
	$(item).find(tfPath['photo_td']).width(rlConfig['pg_upload_thumbnail_width']);
	$(item).height(rlConfig['pg_upload_thumbnail_height'] + 30);
	...
	for ( var i = 0; i < fields.length; i++ )
	{
		...
		set_class += i > 1 ? 'hide' : '';
		var new_row = '<tr class="'+ set_class +'">'+name+'<td class="value'+first+'">'+trim(fields[i], ' ,')+'</td></tr>';
	...
	/* move category */
	$(item).find('div.top-layer > div.nav').prepend(category);
...
$(document).ready(function(){
	...
	/* move "more menu" container and set click handler */
	$('#main_menu li.more').append($('#main_menu_more'));
