		...
		<div id="header">
			...
			<div id="header-nav">
				<div class="inner">
					...
					<ul class="cat_alphabet">
						{foreach from=','|explode:$lang.alphabet_characters item='character'}
							{if $character|count_characters == 1}
								<li><a href="javascript:void(0)" rel="nofollow">{$character}</a></li>
							{/if}
						{/foreach}
						<li><a href="javascript:void(0)" rel="nofollow">#</a></li>
					</ul>
					<div class="hide" id="cat_alphabet_cont"><div class="loading hide">{$lang.loading}</div><img class="close" alt="{$lang.cancel}" title="{$lang.cancel}" src="{$rlTplBase}img/blank.gif" /></div>
					<script type="text/javascript">//<![CDATA[
					var nothing_found_for_char = "{$lang.nothing_found_for_char}";
					{literal}
					$(document).ready(function(){
						catAlpha();
					});
