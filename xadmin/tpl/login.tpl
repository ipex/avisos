...
<script type="text/javascript" src="{$rlBase}js/md5.js"></script>
					...
					{else}
						<form name="login" action="" method="post" onsubmit="return false;">
							<div class="relative" style="margin: 0 0 5px">
								<input style="width: 215px;" maxlength="25" type="text" id="username" name="username" />
								<div id="username_hint" class="input_hint">{$lang.username}</div>
							</div>
							
							<div class="relative" style="margin: 0 0 5px">
								<input style="width: 215px;" maxlength="25" type="password" id="password" name="password" />
								<div id="password_hint" class="input_hint">{$lang.password}</div>
							</div>
...
var sec_key = "{$config.security_key}";
var lang = new Array();
...
$(document).ready(function(){
	...
	$('#username').focus(function(){
		$('#username_hint').fadeOut();
	}).click(function(){
		$('#username_hint').fadeOut();
	}).keydown(function(){
		$('#username_hint').fadeOut('fast');
	}).blur(function(){
		if ( $(this).val() == '' && !$('#username_hint').is(':visible') )
		{
			$('#username_hint').fadeIn();
		}
	});
	
	$('#password').focus(function(){
		$('#password_hint').fadeOut();
	}).blur(function(){
		if ( $(this).val() == '' )
		{
			$('#password_hint').fadeIn();
			eventListener();
		}
	});
	
	$('#username_hint').click(function(){
	...
	/* chrome yellow autocomplete fix */
...
function eventListener()
{
	if ( $('#password').val() != '' )
	{
		$('#password_hint').fadeOut('fast');
		return;
	}
	listener_time = setTimeout('eventListener()', 400);
}
