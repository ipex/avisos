	...
	<tr>
		...
		<td class="list-body changelog_{$log_item.status}">
			<div class="changelog_item" id="pChangelog_{$pLog_key}">
				<a class="green_14" target="_blank" href="http://www.flynax.com/plugins/{$log_item.key|lower}.html#changelog" title="{$lang.learn_more_about} {$log_item.name}">{$log_item.name}</a>
				...
				{else}
					{if $log_item.status != 'no'}
						<a {if $log_item.status == 'update'}title="{$lang.update_from_to|replace:$update_from:$log_item.current|replace:$update_to:$log_item.version}"{/if} name="{$log_item.key}" href="javascript:void(0)" class="{$log_item.status}_icon remote_{$log_item.status}"><span></span>{if ($log_item.version.0 > $log_item.current.0) && $log_item.current.0}{$lang.upgrade}{else}{$lang[$log_item.status]}{/if}</a>
					{else}
