			...
			<tr>
				<td style="width: 110px;" valign="top">
					<a href="{$rlBase}index.php?controller=accounts&amp;action=view&amp;id={$contact.ID}" target="_blank" title="{$lang.account_information}">
						...
						{else}
							<img class="thumbnail" alt="" src="{$smarty.const.RL_URL_HOME}files/{$contact.Photo}" style="width: 66px;height: auto;" />
						{/if}
					</a>
					...
					<td valign="top">
						<a href="{$rlBase}index.php?controller={$cInfo.Controller}&amp;id={$item.From}{if $item.Admin == $item.From}&amp;administrator{/if}" title="{$lang.chat_with} {$item.Full_name}">
							{if empty($item.Photo)}
								<img class="avatar70" alt="" src="{$rlTplBase}img/blank.gif" />
							{else}
								<img class="thumbnail" alt="" src="{$smarty.const.RL_URL_HOME}files/{$item.Photo}" style="width: 66px;height: auto;" />
							{/if}
						</a>
