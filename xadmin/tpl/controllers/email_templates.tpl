...
{if $smarty.get.action}
	...
	<form action="{$rlBaseC}action={if $smarty.get.action == 'add'}add{elseif $smarty.get.action == 'edit'}edit&amp;tpl={$smarty.get.tpl}{/if}" method="post">
	...
	$(document).ready(function(){
		$('.caret_button').click(function(){
			...
			else
			{
				...
				var new_text = text.substring(0, offset.startOffset) + variable + text.substring(offset.endOffset, text.length);
				offset.startContainer.setText(new_text)
			}
		});
	});
