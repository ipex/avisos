...
<div class="middle_right">
	...
	<!-- print bread crumbs -->
	<div id="bc_container">
		{if $cInfo.Controller != 'home'}<a href="{$rlBase}index.php" class="bread_crumbs">{$lang.admin_panel}</a>{/if}{if $cInfo.Controller != 'home'}{foreach from=$breadCrumbs item='bc' name='bc_foreach'}{if $smarty.foreach.bc_foreach.last}<a href="{$rlBase}index.php?controller={if empty($bc.Controller)}{$smarty.server.QUERY_STRING|replace:'controller=':''|replace:'&':'&amp;'}{else}{$bc.Controller}{/if}{if $bc.Vars}&amp;{$bc.Vars}{/if}" class="current">{$bc.name}</a>{assign var='pTitle' value=$bc.name}{else}<a href="{$rlBase}index.php?controller={$bc.Controller}{if $bc.Vars}&amp;{$bc.Vars}{/if}">{$bc.name}</a>{/if}{/foreach}{else}{assign var='pTitle' value=$breadCrumbs.1.name}{/if}
	</div>
	{if !empty($pTitle)}
	...
	{/literal}
		{if $pNotice}
		printMessage('notice', '{$pNotice}');
		{elseif $alerts}
		printMessage('alert', '{if is_array($alerts)}<ul>{foreach from=$alerts item="alert"}<li>{$alert}</li>{/foreach}</ul>{else}{$alerts}{/if}');
		{elseif $errors}
		printMessage('error', '{if is_array($errors)}<ul>{foreach from=$errors item="error"}<li>{$error}</li>{/foreach}</ul>{else}{$errors}{/if}');
		{elseif $infos}
		printMessage('info', '{if is_array($infos)}<ul>{foreach from=$infos item="info"}<li>{$info}</li>{/foreach}</ul>{else}{$infos}{/if}');
		{/if}
	{literal}
	});
